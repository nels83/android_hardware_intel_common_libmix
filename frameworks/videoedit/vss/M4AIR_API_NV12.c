...
#define M4AIR_NV12_FORMAT_SUPPORTED
#define M4AIR_NV12A_FORMAT_SUPPORTED
...
M4OSA_ERR M4AIR_create_NV12(M4OSA_Context* pContext,M4AIR_InputFormatType inputFormat)
{
    ...
    *pContext = M4OSA_NULL ;
    ...
    switch(inputFormat)
    {
#ifdef M4AIR_NV12_FORMAT_SUPPORTED
        case M4AIR_kNV12P:
        break ;
#endif
#ifdef M4AIR_NV12A_FORMAT_SUPPORTED
        case M4AIR_kNV12AP:
        break ;
#endif
    ...
    else
    {
        ...
        for(i=0;i<nb_planes;i++)
        {
            ...
            /**< Take into account that X coordinate can be odd
                in this case we have to put a 0.5 offset
                for U and V plane as there a 2 times sub-sampled vs Y*/
            if((pC->m_params.m_inputCoord.m_x&0x1)&&(i==1))
            {
                pC->u32_x_accum_start[i] += 0x8000;
            }

            M4OSA_TRACE1_4("u32_x_inc = 0x%x, u32_y_inc = 0x%x, u32_x_accum_start = 0x%x, u32_y_accum_start = 0x%x",\
                pC->u32_x_inc[i], pC->u32_y_inc[i], \
                pC->u32_x_accum_start[i], pC->u32_y_accum_start[i]);
        }
    }
 ...
 ******************************************************************************
 */
M4OSA_ERR M4AIR_get_NV12(M4OSA_Context pContext,
    M4VIFI_ImagePlane* pIn, M4VIFI_ImagePlane* pOut)
{
    M4AIR_InternalContext* pC = (M4AIR_InternalContext*)pContext ;
    ...
    M4OSA_TRACE1_0("M4AIR_get_NV12 start");
    ...
    for(i=0;i<nb_planes;i++)
    {
        ...
        u32_shift = ((i==0)||(i==2))?0:1; /**< Depend on Luma or Chroma */

        M4OSA_TRACE1_3("Plane %d: m_x = %d, m_y = %d",i,
            pC->m_params.m_inputCoord.m_x,pC->m_params.m_inputCoord.m_y);
            ...
            ||((M4OSA_TRUE == pC->m_params.m_bOutputStripe)&&(0 == pC->m_procRows)))
        {
            ...
            pu8_data_in = pIn[i].pac_data + ((pIn[i].u_topleft >> u32_shift) << u32_shift) + 
                ((pC->m_params.m_inputCoord.m_x >> u32_shift) << u32_shift)
            ...
            {
                pu8_data_in += pC->m_params.m_inputSize.m_width -1 ;
            }
        ...
        pu8_data_out    = pOut[i].pac_data + ((pOut[i].u_topleft >> u32_shift) << u32_shift);

        M4OSA_TRACE1_2("pOut[%d].u_topleft = %d",i,pOut[i].u_topleft);
                ...
                if(M4OSA_FALSE == pC->m_bRevertXY)
                {
                    ...
                    for(j=0;j<pOut[i].u_height;j++)
                    {
                        ...
                        if(M4OSA_TRUE ==  pC->m_bFlipX)
                        {
                            ...
                            /**< Loop on each output pixel in a row */
                            for(k=0;k<pOut[i].u_width;k+=2)
                            {
                                // handle U plane
                                u32_x_frac = (u32_x_accum >> 12)&15; /* Fraction of Horizontal
                                                                        weight factor */

                                pu8_src_top = (pu8_data_in - ((u32_x_accum >> 16) << 1)) -2 ;

                                pu8_src_bottom = pu8_src_top + i32_tmp_offset;

                                /* U plane weighted combination */
                                u32_temp_value = (M4VIFI_UInt8)(((pu8_src_top[2]*(16-u32_x_frac) +
                                                   pu8_src_top[0]*u32_x_frac)*(16-u32_y_frac) +
                                                   (pu8_src_bottom[2]*(16-u32_x_frac) +
                                                   pu8_src_bottom[0]*u32_x_frac)*u32_y_frac )>>8);
                    ...
                    for(j=0;j<pOut[i].u_height;j++)
                    {
                        ...
                        if(M4OSA_TRUE ==  pC->m_bFlipX)
                        {
                            ...
                            for(k=0;k<pOut[i].u_width;k+=2)
                            {
                                ...
                                pu8_src_top = (pu8_data_in - ((pC->u32_x_accum[i] >> 16) << 1)) - 2;
