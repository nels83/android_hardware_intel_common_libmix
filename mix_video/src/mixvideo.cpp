...
MIX_RESULT mix_video_get_version_default(MixVideo * mix, uint * major,
        uint * minor);
...
MIX_RESULT mix_video_get_max_coded_buffer_size_default (MixVideo * mix, uint *max_size);
...
MIX_RESULT mix_video_get_new_userptr_for_surface_buffer_default (MixVideo * mix, uint width, uint height, uint format,
...
MixVideo::MixVideo() {
    ...
    get_new_usrptr_for_surface_buffer = mix_video_get_new_userptr_for_surface_buffer_default;
    mix_video_init(this);
...
void mix_video_private_initialize(MixVideoPrivate* priv) {
    ...
    /*
    * usrptr shared buffer mode
    */
    priv->requested_surface_info.surface_cnt = 0;
    memset (priv->requested_surface_info.surface_allocated, 0 , sizeof (uint) * MAX_ENC_SURFACE_COUNT);
    memset (priv->requested_surface_info.usrptr, 0 , sizeof (uint8 *) * MAX_ENC_SURFACE_COUNT);
}
...
MIX_RESULT mix_video_configure_encode(MixVideo * mix,
    ...
    ret = mix_videofmtenc_initialize(priv->video_format_enc,
                                     ...
                                     &(priv->requested_surface_info), priv->va_display);
...
MIX_RESULT mix_video_render_default(MixVideo * mix,
    ...
    LOG_V( "Displaying surface ID %d, timestamp %"UINT64_FORMAT"\n", (int)va_surface_id, timestamp);
    ...
    case MIX_ENC_PARAMS_RESOLUTION:
    {
        ...
        */
        ...
        if (params_type == MIX_ENC_PARAMS_RESOLUTION) {
            ...
            LOG_E("set dynamic_params->rc_mode = %d", dynamic_params->rc_mode);
        ...
        uint expected_size, uint *outsize, uint * stride, uint8 **usrptr)
{
    ...
    /*
    * If mixvideo has been configured, we can not request surface creation anymore
    */
    if (priv->configured) {
        LOG_E( "Already configured, can not request VA surface anymore\n");
        return MIX_RESULT_WRONG_STATE;
    }
    ...
    */
    if (format != MIX_STRING_TO_FOURCC("NV12")) {
    ...
    if (priv->requested_surface_info.surface_cnt >= MAX_ENC_SURFACE_COUNT) {
    ...
    va_status = vaDeriveImage(priv->va_display, surface, &image);
    if (va_status != VA_STATUS_SUCCESS) {
    ...
    //priv->src_image [priv->alloc_surface_cnt] = image;

    va_status = vaMapBuffer (priv->va_display, image.buf, (void **) usrptr);
    if (va_status != VA_STATUS_SUCCESS) {
    ...
    *stride = image.pitches[0];
    ...
    priv->requested_surface_info.surface_allocated[priv->requested_surface_info.surface_cnt] = surface;
    priv->requested_surface_info.usrptr[priv->requested_surface_info.surface_cnt] = *usrptr;
    ...
    LOG_I("data_size = %d\n", image.data_size);
    LOG_I("usrptr = 0x%08x\n", *usrptr);
    LOG_I("surface_cnt = %d\n", priv->requested_surface_info.surface_cnt);
    LOG_I ("priv->usrptr[%d] = 0x%08x\n ",
           priv->requested_surface_info.surface_cnt,
           priv->requested_surface_info.usrptr[priv->requested_surface_info.surface_cnt]);

    va_status = vaUnmapBuffer(priv->va_display, image.buf);
    if (va_status != VA_STATUS_SUCCESS) {
        LOG_E("Failed to vaUnmapBuffer\n");
        goto cleanup;
    }

    va_status = vaDestroyImage(priv->va_display, image.image_id);
    if (va_status != VA_STATUS_SUCCESS) {
        LOG_E("Failed to vaDestroyImage\n");
        goto cleanup;
    }

    if (*outsize < expected_size) {
        LOG_E ("Allocated buffer size is small than the expected size, destroy the surface");
        LOG_I ("Allocated size is %d, expected size is %d\n", *outsize, expected_size);
        va_status = vaDestroySurfaces(priv->va_display, &surface, 1);
        goto cleanup;
    }
...
MIX_RESULT mix_video_get_new_userptr_for_surface_buffer (MixVideo * mix, uint width, uint height, uint format,
        uint expected_size, uint *outsize, uint * stride, uint8 **usrptr) {
    return mix->get_new_usrptr_for_surface_buffer(mix, width, height, format, expected_size, outsize, stride, usrptr);
