...
gboolean MixBufferPool::copy(MixParams * target) const {
	gboolean ret = FALSE;
...
gboolean MixBufferPool::equal(MixParams * obj) const {
	gboolean ret = FALSE;
...
MIX_RESULT mix_bufferpool_initialize(
	MixBufferPool * obj, guint num_buffers) {
	LOG_V( "Begin\n");
...
mix_bufferpool_dumpbuffer(MixBuffer *buffer) {
	LOG_I( "\tBuffer %x, ptr %x, refcount %d\n", (guint)buffer,
		(guint)buffer->data, MIX_PARAMS(buffer)->ref_count);
	return MIX_RESULT_SUCCESS;
}
