...
MIX_RESULT MixVideoFormat_MP42::_initialize_va(vbp_data_mp42 *data) {
	...
	this->va_surfaces = reinterpret_cast<VASurfaceID*>(g_malloc(sizeof(VASurfaceID)*this->va_num_surfaces));
...
MIX_RESULT MixVideoFormat_MP42::_decode_end(gboolean drop_picture) {
	MIX_RESULT ret = MIX_RESULT_SUCCESS;
	VAStatus vret = VA_STATUS_SUCCESS;
	...
	if (pic_data->vop_coded == 0) {
		...
		if (this->next_nvop_for_PB_frame) {
			...
			LOG_V("Processing skipped frame %x, frame_id set to %d, ts %"G_GINT64_FORMAT"\n",
				(guint)skip_frame, (guint)frame_id, this->current_timestamp);
...
MIX_RESULT MixVideoFormat_MP42::Decode(
	MixBuffer * bufin[], gint bufincnt, MixVideoDecodeParams * decode_params) {
