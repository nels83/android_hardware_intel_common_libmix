...
mix_videoformatenc_h264_new(void) {
    MixVideoFormatEnc_H264 *ret = reinterpret_cast<MixVideoFormatEnc_H264*>(
...
#define USE_SRC_FMT_NV21
    ...
    if (iovout->data == NULL) { //means  app doesn't allocate the buffer, so _encode will allocate it.
        iovout->data = reinterpret_cast<guchar*>(g_malloc (size));  // In case we have lots of 0x000001 start code, and we replace them with 4 bytes length prefixed
        if (iovout->data == NULL) {
    ...
    if (mix->delimiter_type == MIX_DELIMITER_ANNEXB) {
        ...
        guint8 * payload = reinterpret_cast<guint8*>(coded_seg->buf);
        ...
        if ((payload [pos - 1] & 0x01) && mix->slice_num == 1 && nal_unit_type == 1 && num_seg == 1) {
            ...
            if (num_seg == 1) {
                ret = mix_videofmtenc_h264_AnnexB_to_length_prefixed (reinterpret_cast<guint8*>(coded_seg->buf), coded_seg->size, iovout->data, &size);
                if (ret != MIX_RESULT_SUCCESS)
                {
                    LOG_E (
                ...
                guint8 * tem_buf = NULL;
                tem_buf = reinterpret_cast<guint8*>(g_malloc (size));
                if (tem_buf == NULL) {
                    LOG_E( "tem_buf == NULL\n");
                    ret = MIX_RESULT_NO_MEMORY;
                    CLEAN_UP;
